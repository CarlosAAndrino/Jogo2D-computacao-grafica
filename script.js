...
function desenhar(tempoAtual) {
    ...
    if (deltaTempo > 1000 / velocidadeCobra) {
        ...
        snake.forEach(segment => {
            ctx.fillStyle = 'green';
            ctx.beginPath();
            ctx.arc(segment.x * tamanhoCelula + tamanhoCelula / 2, segment.y * tamanhoCelula + tamanhoCelula / 2, tamanhoCelula / 2, 0, Math.PI * 2);
            ctx.fill();
        });

        ctx.fillStyle = 'red';
        ctx.fillRect(comida.x * tamanhoCelula, comida.y * tamanhoCelula, tamanhoCelula, tamanhoCelula);

        if (
            novaCabeca.x < 0 || novaCabeca.x >= canvas.width / tamanhoCelula ||
            novaCabeca.y < 0 || novaCabeca.y >= canvas.height / tamanhoCelula
        ) {
            encerrarJogo();
        }
